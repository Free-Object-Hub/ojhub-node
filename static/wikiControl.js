...
wikiSetColors = (wikiId, colorScheme = '')=>{
	...
	helperRequest(`${sData[7]}colors${php}`, `wiki=${wikiId}&color=${colorScheme}`)
		.then(data=>{
			...
			_.$.id('wikiColor'+wikiId).setAttribute('onclick', `realColorGenerator(${wikiId},'${colorScheme}')`);
...
uploadFilesWindow = (wikiId)=>{
	_.win.open('uploadFiles',
		...
		`<form id={winId}formFile onsubmit="return uploadFiles(this,${wikiId},{winId})">`+
...
editTemplateWindow = (wikiId, templateName = '')=>{
	...
	let subWindowId = _.win.open('templateEditor',
		...
		`<div>`+
			...
			basicButton('>+<', `_.$.id('${templateName}-args').insertAdjacentHTML('beforeend', templateArg(''))`, 'width:calc(100% - 8px)')+
