...
$ = {
	...
	link: { // warning!! use $.cfg.link.actions for pages, for modal windows and other use $cfg.linkCommands instead
		ignore: false,
		compileLink() {
			return location.search.replace('?','').split('&');
		},
		set(val, pageTitle = $.cfg.link.defaultTitle) {
			if (pageTitle)
		...
		get() {
			...
			this.compileLink().forEach(keyPre => {
				...
				try {
					let mainRoute = $.cfg.link.actions[key],
						cmdRoute = $.cfg.link.commands[key];
					if (isFirst)
						if (mainRoute)
							mainRoute(value);
						else 
					...
					console.error(e);$.err.handleRejection(e);;
					if (isFirst)
						basePage();
				}
				isFirst = false;
	...
	lang: {
		main: {},
		load(name, autoSetup = true) {
			return new Promise((resolve, reject)=>{
				helperRequest($.cfg.lang.addr+name+'.json', false, {'Cache-Control':'no-cache, no-store, max-age=0'})
					.then(data=>{
						let packet = data;
						if (autoSetup)
							packet = this.parse(packet);
						$.lang.main = JSON.parse(packet);
						resolve($.lang.main);
					})
			})
		},
		parse(packet) {
			$.cfg.lang.vars.forEach(varl => {
            	const getter = new Function(`return ${varl}`);
            	const value = getter();
				packet = packet.replaceAll(`+${varl}+`, value);
			});
			return packet;
		},
		async replace(langName) {
			return this.load(langName, true)
				.then(p=>{
					console.time('nh-trans');
					let dataTrans = $qa('[data-trans]');
					for (let el of dataTrans) {
						let key = el.getAttribute('data-trans'),
						langValue = $.lang.main[key];
						if (langValue == '')
					...
					console.timeEnd('nh-trans');
					return $.lang.main;
				})
		},
		get(id, renderAs = 'text') {
			let getText = $.lang.main[id],
			...
			if (getText == undefined || getText == '')
				if (renderAs == 'window') {
					...
					getText = `<code>getTrans('${id}','${renderAs}')</code>`;
		...
		register(script, funcs) {
			...
			funcs.forEach(fn=>{
				window[fn] = async (...args) => {
					let func = await $.lazy.l(script, fn);
					return func(...args);
				}
			});
	...
	err: {
		...
		show(errID, errText, addr = '') {
			...
			let buttons = '';
			$.cfg.err.errorWindowButtons.forEach(btn=>{
		...
		handleRejection(e) {
			...
			let f = err.stack.split('\n')[0],
				errSrc = f.split(location.origin + location.pathname)[1].split('?')[0],
				errPos = f.split('&helper:')[1].split(':');
			$.err.log(
				`PROMISE ERROR\n`+
				`${e}`+
				`\nIN ${errSrc} ON LINE ${errPos[0]} IN COLUMN ${errPos[1]}`
			);
		},
	},
};
...
// #region legacy
loadScript = (u, ...a) => {
	console.warn($.title+'> loadScript is deprecated! use $.lazy.load instead');
	return $.lazy.load(u,...a);
}

getTrans = (i, r = 'text') => {
	console.warn($.title+'> getTrans is deprecated! use $.lang.get instead');
	return $.lang.get(i,r);
}
// #endregion
// #region subWindows
...
subWindows = {
	...
	initDragger: (subWinId)=>{
		...
		startDrag = e=>{
			...
			helperWindows.appendChild(subWindow);
	...
	hide: (subWinId, subWinName)=>{
		...
		subWindow.onanimationend = ()=>{
			...
			helperHider.insertAdjacentHTML('beforeend', basicButton(`${getTrans('WINDOW-'+subWinName,'window')}`, `subWindows.unhide('${subWinId}')`, ``, `hider${subWinId}`));
			subWindow.onanimationend = null;
		};
