...
removeApl = (id, gdpsId, winId)=>{
		Loading();
		//helperRequest(`${sData[8]}removeApl${php}?id=${id}&gdpsId=${gdpsId}`)
	...
	if (isPhone == 0) 
		html = 
			`<div class=ANIM-swticherUp id=userSettings2>`+
				...
				`<button class=loginbtn onclick="innerProfile();keyBindsCfg()"${getTrans('settings013')}/button><br><br>`+
			`</div>`;
	else 
		html = 
			`<div class=ANIM-swticherUp id=userSettings2 style="display:grid;margin:0 8px 0 8px">`+
				...
				`<button class=loginbtn onclick="innerProfile();keyBindsCfg();profileSwitcherPhone()"${getTrans('settings013')}/button>`+
			`</div>`;
	return html;
},
...
editFind = (channel, gdpsId)=>{
	...
	//helperRequest(`${nData[10])}edit?id=${gdpsId}`)
...
editVacs = (channel, gdpsId, vacId)=>{
	...
	let html = ``,
			smallString = 'vacs',
			bigString = 'Vacs';
	//helperRequest(`${nData[8]}edit?id=${vacId}&gdpsId=${gdpsId}`)
