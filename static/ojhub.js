...
helperCaptchaSiteKey = '6LeIxAcTAAAAAJcZVRqyHh71UMIEGNQ_MXjiZKhI',//'6Ldrt0grAAAAAMdteG7pq6LZ1UYeMvkElvUV7Qhx',
globalWiki = 0,
wikiTemplates = {},
...
thisUser = Slocal.get('StaticUserData') ? JSON.parse(Slocal.get('StaticUserData')) : {
	username: 'Object Hub',
	ID: 0,
	role: 0,
	isActive: 0,
...
let reStart = (drop = 0)=>{
	let helperInit = ()=>{
		...
		Fingerprint.generate()
			.then(fpData=>{
				fp = fpData;
				_.http.defaultHeaders['device-static'] = fp.staticName;
				LIKES.init();
...
nodeServer = 'http://127.0.0.1/api/v1',
nData = [
	nodeServer+'/content/',
	nodeServer+'/send/',
	nodeServer+'/',
...
contentRender = function(
	...
	let html = `<div class=framegdps ${joinData ? `id=news${preHtml.ID}` : ''} ${isComm == 0 ? 'style="width:calc(100% - 40px)"' : ''}>`+
		(preHtml.img || tags ?
		`<img loading=lazy onerror="console.warn('broken link');this.src='${helperUrl}imgs/hubbig.png'" align="left" src="${decodeURIComponent(preHtml.img)}" width=128px height=128px style="border-radius:calc(var(--def-border)*2)">`
		: '')+
...
contentRenderMinu = function(
	...
	`<div class="framegdps" styling="${btnFuncs[2]}${data.ID}" style="${preHtml[3]}">`+
		(renderImage ? `<div class=loh style="min-height:128px">` : '')+
			...
			`</p>` : '')+
			(renderImage ? `<img loading=lazy onerror="console.warn('broken link ${decodeURIComponent(data.img)}');this.src='${helperUrl}imgs/hubbig.png'" align="left" src="${decodeURIComponent(data.img)}" width=128px height=128px style="border-radius:calc(var(--def-border)*2)">`+
		`</div>` : '')+
...
getVacsWithComments = (vacId)=>{
	...
	Loading();
	//helperRequest(`${sData[0]}vacsC${php}?id=${vacId}`)
...
sendRegisterForm = (wId)=>{
	...
	if (!ignoreCap && !reCAP) {
		...
		Loading();
		helperRequest(`${sData[5]}register${php}`,
			`username=${username}&password=${password}&email=${email}&g-recaptcha-response=${reCAP}`+fp.urlDone,
...
sendLoginForm = (wId)=>{
	...
	if (!ignoreCap && !reCAP) {
		...
		Loading();
		//helperRequest(`${sData[5]}login${php}`,`username=${username}&password=${password}&g-recaptcha-response=${reCAP}${fp.urlDone}`)
		_.http.req('POST', `${nData[5]}login`,
			`username=${username}&password=${password}&g-recaptcha-response=${reCAP}`+fp.urlDone,
		{'Content-type':'application/x-www-form-urlencoded'})
			.then(data=>{
				Loading(1);
				switch (data) {
...
sendComm = (id, channel, likeChannel)=>{
	...
	_.http.req('POST', `${nData[9]}send`, data, {'Content-type':'application/x-www-form-urlencoded'})
...
modifyComm = (id, channel)=>{
	...
	Loading();
	_.http.req('POST', `${nData[9]}modify`, data)
		...
		`<div style="background-color:var(--color-profile)">`+
			`<div class=contentAdaptiveFlexSmall style=position:relative;align-items:center;justify-content:center;overflow:hidden>`+
				...
				`<div style=position:relative;width:35vw;height:370px;z-index:1>`+
					`<img src="${helperUrl}imgs/grad.png"  height=1000px style="position:absolute;right:-220px;top:-100px;transform:rotate(-3deg)">`+

					`<img src="${helperUrl}imgs/gem.png"   height=322px style="filter:drop-shadow(2px 2px 8px #000);position:absolute;right:420px;top:100px">`+
					`<img src="${helperUrl}imgs/gasi.png"  height=348px style="filter:drop-shadow(2px 2px 8px #000);position:absolute;right:270px;top:50px;transform:rotate(-3deg)">`+
					`<img src="${helperUrl}imgs/share.png" height=424px style="filter:drop-shadow(2px 2px 8px #000);position:absolute;right:-20px;top:-11px;transform:rotate(0deg)">`+
				`</div>`+
			`</div>`+
			`<div class=contentAdaptiveSmall align=center style=position:relative;overflow:hidden>`+
				`<div class=textFly style=z-index:2>`+
					`<img src="${helperUrl}imgs/grad.png"  height=1000px style="position:absolute;right:-180px;top:-100px;transform:rotate(-3deg)">`+
...
pageFind = (channel = 1)=>{
	...
	let html = pHeader()+
	`<div id=helperContent>`+
		`<h1 align=left style=color:var(--color-white);margin-left:12px${getTrans('ojhubname')}/h1>`+
...
globalVacs = ()=>{
	...
	//helperRequest(`${sData[8]}getAll${php}?page=0`)
		.then(data => {
			Loading(1);
			let parsedData = JSON.parse(data);
...
gdpsReport = (gdpsId)=>{
	_.win.open('REPform',
		...
		<form id={winId}formREP onsubmit="return enterFormData(this,'report${php}')">
			<input type=hidden value="{winId}REPform" name=windowId>
...
RenderNews = (data, isComm = 0, backFunc = 'getCamp', commBackFunc = '')=>{
	...
	for (let ide in data)	{
		...
		html += `<div class=framegdps id=news${gdpsData.ID} ${widthCfg[isComm]}>`+
...
vacRespond = (vacId)=>{
	Loading();
	//helperRequest(`${sData[8]}apply${php}?id=${vacId}`)
...
vacUnrespond = (vacId, aplId)=>{
	Loading();
	//helperRequest(`${sData[8]}removeApl${php}?id=${aplId}&vacId=${vacId}`)
...
makeBetaAlert = ()=>{
	helperMain.insertAdjacentHTML('afterend',
		`<div class=ALERT id=BETAalert style=position:absolute;top:20%;left:50%><h1>BETA!</h1>`+
			...
			`<button style=background-color:#333 onclick="Slocal.set('BetaRead',1);_.$.id('BETAalert').remove()">Понятно</button>`+
		`</div>`
	);
},
...
WIKIrenderInProfile = (parsedData)=>{
	...
	for (let Id in parsedData) {
		...
		// html += 
		// `<div class=framegdps style="width:300px;height:250px" id="${id}">`+
		//	 `<img width=316px height=133px src="${guidImg}" onerror="console.warn('broken link');this.src='${helperUrl}imgs/hubemp.png'"`+
		//		 `style="position:absolute;top:0;left:0;margin:0;border-top-left-radius:var(--def-border);border-top-right-radius:var(--def-border)">`+
		//	 `<h2 style="z-index:1;position:inherit;margin-top:120px">${guidTitle} <img class=FGDPSimg src="${helperUrl}imgs/${guidLang}.png"></h2>`+
		//	 `<div style="position: absolute;top: 0;left: 0;width: 316px;height:60px;margin-top:73px;background: linear-gradient(rgba(0,0,0,0), var(--color-profile-alpha), var(--color-profile));"></div>`+
		//	 `<div style="bottom:12px;left:20px" class="absolute btnszone">`+
		//		 `<button onclick="pageGuides(${id})" class=loginbtn${getTrans('moreInfo')}/button>`+
		//	 `</div>`+
		// `</div>`;
	};
	return html;
...
doHotkeyAction = (keyCode)=>{
	const profileElementHas = !!_.$.id('profileWindow'),
		keysList = {
			[keyActions.profile]: ()=>{
				if (!profileElementHas)
					profilePage();
				else
					innerProfile(gProfileMini());
			},
			[keyActions.yourCamps]: ()=>{
				if (!profileElementHas) profilePage('');
				findsWindow(0);
			},
			[keyActions.yourShows]: ()=>{
				if (!profileElementHas) profilePage('');
				findsWindow(1);
			},
			[keyActions.yourPeres]: ()=>{
				if (!profileElementHas) profilePage('');
				findsWindow(2);
			},
			[keyActions.addNews]: ()=>{
			...
			[keyActions.yourWikis]: ()=>{
				if (!profileElementHas) profilePage('');
				wikisWindow();
			},
			[keyActions.act0]: ()=>{
				parseCustomAction(keyCustomActions[0]);
			},
			[keyActions.act1]: ()=>{
				parseCustomAction(keyCustomActions[1]);
			},
			[keyActions.act2]: ()=>{
				parseCustomAction(keyCustomActions[2]);
			},
			[keyActions.act3]: ()=>{
				parseCustomAction(keyCustomActions[3]);
			},
			[keyActions.act4]: ()=>{
				parseCustomAction(keyCustomActions[4]);
			},
			[keyActions.act5]: ()=>{
				parseCustomAction(keyCustomActions[5]);
			},
			[keyActions.act6]: ()=>{
				parseCustomAction(keyCustomActions[6]);
			},
			[keyActions.act7]: ()=>{
				parseCustomAction(keyCustomActions[7]);
			},
			[keyActions.act8]: ()=>{
				parseCustomAction(keyCustomActions[8]);
			},
			[keyActions.act9]: ()=>{
				parseCustomAction(keyCustomActions[0]);
			},
		}
	let keyEvent = keysList[keyCode];
	console.log(keysList[keyCode]);
	if (keyEvent)
		keyEvent();
},
parseCustomAction = (actStr)=>{
	if (actStr == '')
		return;
	let [command,Id] = actStr.split('!');
	let [Id1,Id2] = ['',''];
	if (Id)
		[Id1,Id2] = Id.split('.')
	switch (command) {
		...
		case 'd':
			return getPere(Id);
		case 'ce':
			if (!_.$.id('helperContentProfile'))
				profilePage('');
			return editFind(0,Id);
		case 'se':
			if (!_.$.id('helperContentProfile'))
				profilePage('');
			return editFind(1,Id);
		case 'de':
			if (!_.$.id('helperContentProfile'))
				profilePage('');
			return editFind(2,Id);
		case 'nc':
			return helperNews(Id+'.');
		case 'ns':
			return helperNews(Id+',');
		case 'nd':
			return helperNews(Id+'/');
		case 'vc':
			if (!_.$.id('helperContentProfile'))
				profilePage('');
			return getVacancies(0,Id);
		case 'vs':
			if (!_.$.id('helperContentProfile'))
				profilePage('');
			return getVacancies(1,Id);
		case 'vd':
			if (!_.$.id('helperContentProfile'))
				profilePage('');
			return getVacancies(2,Id);
		case 'vac':
			if (!_.$.id('helperContentProfile'))
				profilePage('');
			return vacResponses(0,Id1,Id2);
		case 'vas':
			if (!_.$.id('helperContentProfile'))
				profilePage('');
			return vacResponses(1,Id1,Id2);
		case 'vad':
			if (!_.$.id('helperContentProfile'))
				profilePage('');
			return vacResponses(2,Id1,Id2);
			
		case 'w':
			return pageGuides(Id);
		case 'wp':
			return getGuide(Id1,Id2);
		case 'wc':
			if (!_.$.id('helperContentProfile'))
				profilePage('');
			return wikiControl(Id);
		case 'we':
			if (helperSettings.openGuidesInWindow == 0) {
				if (!_.$.id('helperContentProfile'))
					profilePage('');
				if (!_.$.id('wikiControlP'))
					wikiControl(Id2);
			}
			editGuide(Id1,Id2)
	}
...
keyActions = Slocal.get('KeysCfg') ? JSON.parse(Slocal.get('KeysCfg')) : {
	main: 'ShiftLeft',
	profile: 'KeyQ',
	yourCamps: 'KeyC',
	yourShows: 'KeyS',
...
keyCustomActions = Slocal.get('KeysCustomCfg') ? JSON.parse(Slocal.get('KeysCustomCfg')) : [
	'',
	'',
	'',
	'',
...
keyActionsVer = Slocal.get('KeysCfgVer'); // в перспективе добавить возможность менять shiftLeft на нужную пользователю кнопку
if (!keyActionsVer || keyActionsVer != 4) {
	Slocal.set('KeysCfgVer',           4);
	Slocal.set('KeysCfg', JSON.stringify({
		main: 'ShiftLeft',
		profile: 'KeyQ',
		yourCamps: 'KeyC',
		yourShows: 'KeyS',
	...
	//Slocal.set('KeysCustomCfg', JSON.stringify([
	//	'',
	//	'',
	//	'',
	//	'',
...
window.addEventListener("keydown", e=>{
	const bannedElems = ['INPUT', 'TEXTAREA', 'SELECT'];
	if (bannedElems.includes(e.target.tagName))
		return globalKeyPressed = false;
	const Key = e.code;
	if (Key == keyActions.main)
